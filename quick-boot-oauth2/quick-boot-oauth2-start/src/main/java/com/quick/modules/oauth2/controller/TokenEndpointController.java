...
import com.quick.modules.oauth2.req.LoginReq;
...
public class TokenEndpointController {
    ...
    public Result doLogin(@RequestBody LoginReq loginReq) {
        Result<SysUserApiDTO> result = sysUserApi.findByUsername(loginReq.getUsername());
        SysUserApiDTO sysUserApiDTO = result.getData();
        if(sysUserApiDTO!=null){
            if(sysUserApiDTO.getUsername().equals(loginReq.getUsername()) && BCrypt.checkpw(loginReq.getPassword(),sysUserApiDTO.getPassword())) {
                // 第1步，先登录上

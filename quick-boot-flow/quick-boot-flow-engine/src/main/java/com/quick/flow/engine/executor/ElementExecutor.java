...
public abstract class ElementExecutor extends RuntimeExecutor {
    ...
    protected void preExecute(RuntimeContext runtimeContext) throws ProcessException {
        ...
        if (sourceNodeInstance != null) {
            ...
            FlowNodeInstance flowNodeInstance = nodeInstanceDAO.selectBySourceInstanceId(flowInstanceId,
                    sourceNodeInstance.getNodeInstanceId(), nodeKey);
            //reentrant check
            if (flowNodeInstance != null) {
                BeanUtils.copyProperties(flowNodeInstance, currentNodeInstance);
                runtimeContext.setCurrentNodeInstance(currentNodeInstance);
                LOGGER.warn("preExecute reentrant.||nodeInstancePO={}", flowNodeInstance);
                return;
            }
    ...
    protected void preRollback(RuntimeContext runtimeContext) throws ProcessException {
        ...
        if (runtimeContext.getCurrentNodeInstance() == null) {
            ...
            FlowNodeInstance currentFlowNodeInstance = nodeInstanceDAO.selectByNodeInstanceId(flowInstanceId, nodeInstanceId);
            if (currentFlowNodeInstance == null) {
            ...
            BeanUtils.copyProperties(currentFlowNodeInstance, currentNodeInstance);

            String currentInstanceDataId = currentNodeInstance.getInstanceDataId();
            runtimeContext.setInstanceDataId(currentInstanceDataId);
            FlowInstanceData flowInstanceData = instanceDataDAO.select(flowInstanceId, currentInstanceDataId);
            Map<String, InstanceData> currentInstanceDataMap = InstanceDataUtil.getInstanceDataMap(flowInstanceData.getInstanceData());
    ...
    protected ElementExecutor getRollbackExecutor(RuntimeContext runtimeContext) throws ProcessException {
        ...
        FlowNodeInstance sourceFlowNodeInstance = nodeInstanceDAO.selectByNodeInstanceId(flowInstanceId, sourceNodeInstanceId);
        if (sourceFlowNodeInstance == null) {
        ...
        FlowElement sourceNode = FlowModelUtil.getFlowElement(runtimeContext.getFlowElementMap(),
                sourceFlowNodeInstance.getNodeKey());
